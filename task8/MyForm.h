...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			// SurnameColumn
			// 
			this->SurnameColumn->HeaderText = L"Фамилия";
			...
			// MidnameColumn
			// 
			this->MidnameColumn->HeaderText = L"Отчество";
			this->MidnameColumn->Name = L"MidnameColumn";
			// 
			// ExperienceColumn
			// 
			this->ExperienceColumn->HeaderText = L"Стаж";
			this->ExperienceColumn->Name = L"ExperienceColumn";
			this->ExperienceColumn->Width = 50;
			// 
			// PaymentColumn
			// 
			this->PaymentColumn->HeaderText = L"Оклад";
			...
			this->ResultBtn->Text = L"Сохранить результат";
			...
			this->OpenFileBtn->Text = L"Открыть файл";
			...
			this->AddBtn->Text = L"Добавить ряд";
			...
			this->RmvBtn->Text = L"Удалить ряд";
			...
			this->SaveGridBtn->Text = L"Записать таблицу в файл";
			...
			this->TaskLabel->Text = L"В новый файл вывести людей, занимающих определенную должность.";
			// 
			// label1
			// 
			...
			this->label1->Text = L"Работу можно проверить на Input.txt, вложенном в архив";
			// 
			// MyForm
			// 
...
#pragma endregion
		...
		private: System::Void ResultBtn_Click(System::Object^ sender, System::EventArgs^ e) {
				ClearOutput();
				bool ok = GridValidation(gridResult); // Проверяем все необходимые ограничения
				bool ok2 = (ResultTextBox->Text != "");
				if (!ok) {
					errorProvider1->SetError(ResultBtn, "Неверные значения для одной или нескольких ячеек");
				}
				if (!ok2) {
					errorProvider1->SetError(ResultTextBox, "Введите должность");
				}
				if (!ok || !ok2)
					return;
				...
				std::vector<int> vec;
				for (int i = 0; i < gridResult->RowCount; ++i) { // Найдем ряды, в которых должность совпадает введенной
				...
				if (this->saveFileDialog->ShowDialog() == System::Windows::Forms::DialogResult::OK) {
					if ((myStream = saveFileDialog->OpenFile()) != nullptr) { // Свяжем поток с файлом
						System::IO::StreamWriter^ sw = gcnew System::IO::StreamWriter(myStream,
							System::Text::Encoding::GetEncoding(65001)); // UTF-8
						for (int cur : vec) { // Для всех рядов, в которых должность совпадает с введенной - запишем их в файл.
							WriteRow(cur, sw);
							sw->WriteLine(); // Введем символ конца строки
						}
						sw->Close(); // Закроем поток
					}
				}
		...
		private: System::Void OpenFileBtn_Click(System::Object^ sender, System::EventArgs^ e) {
			...
			if (this->openFileDialog->ShowDialog() == System::Windows::Forms::DialogResult::OK) { // Если открыли файл
				...
				if ((myStream = openFileDialog->OpenFile()) != nullptr) { // Если успешно связали поток с файлом
					...
					System::String^ s = "";
					int innerIdx = 0;
					while ((s = sw->ReadLine()) != nullptr && s != "") { // Если следующая считанная строка существует и не равна пустой
						gridResult->Rows->Add(1); // Создаем новый ряд
						int idx = 0;
						int current = 0;
						while (idx != s->Length) { // Проходимся по всей строке и собираем слова в соответствующие ячейки
		...
		private: bool GridValidation(DataGridView^ grid) { // Функция проверки корректности таблицы
			for (int i = 0; i < gridResult->RowCount; ++i) {
				for (int j = 0; j < gridResult->ColumnCount; ++j) {
					...
					bool ok;
					if (j == 3 || j == 4) {
						ok = System::Int32::TryParse(s, tmp); // Если это поля, в которых должны быть числа - проверим это
					}
					else { // Иначе должна получиться строка. Проверим, что она не содержит цифр.
